...
export default class InstrumentsController {
  ...
  static async AddInstrument(req, res, next) {
    // TO TEST AFTER CONSTRAINT RELAXATION
    ...
    try {
      let query =
        'SELECT isDeleted FROM instruments WHERE instrument_name = $1 AND country = $2 AND sector = $3 AND instrument_type = $4 AND currency = $5 AND isTradeable = $6';
      let params = [
        req.body.instrument_name,
        req.body.country,
        req.body.sector,
        req.body.instrument_type,
        req.body.currency,
        req.body.isTradeable,
      ];
      const results = await pool.query(query, params);
      if (results.rowCount > 0) {
        let query =
          'UPDATE instruments SET isDeleted = false, notes = COALESCE($1, notes) RETURNING instrument_id';
        const results = await pool.query(query, notes);
        res
          .status(200)
          .json('Update ID: ' + results.rows[0].instrument_id.toString());
        return;
      }
    } catch (err) {
      console.log(err.stack);
    ...
    try {
      let query =
        'INSERT INTO instruments (instrument_name, country, sector, instrument_type, currency, isTradeable, notes, created_time, modified_time) VALUES ($1, $2, $3, $4, $5, $6, $7, CURRENT_TIMESTAMP(0), CURRENT_TIMESTAMP(0)) RETURNING instrument_id';
      let params = [
        req.body.instrument_name,
        req.body.country,
        req.body.sector,
      ...
      if (results.rowCount == 0) {
        ...
        res
          .status(200)
          .json('Updated ID: ' + results.rows[0].instrument_id.toString());
      }
    } catch (err) {
      console.log(err.stack);
      res.status(500);
  ...
  static async EditInstrument(req, res, next) {
    ...
    let query =
      'UPDATE instruments SET instrument_name = $1, country = $2, sector = $3, instrument_type = $4, currency = $5, isTradeable = $6, notes = COALESCE($7, notes) modified_time = CURRENT_TIMESTAMP(0) WHERE instrument_id = $8';
    try {
      let params = [
        req.body.instrument_name,
        req.body.country,
  ...
  static async SoftDeleteInstrument(req, res, next) {
    ...
    let query =
      'UPDATE instruments SET isDeleted = true modified_time = CURRENT_TIMESTAMP(0) WHERE instrument_id = $1'; // update a softdeleted column
    try {
      const results = await pool.query(query, params);
      if (results.rowCount == 0) {
        res.status(204).json('No rows returned');
  ...
  static async GetModified(req, res, next) {
    let params = [req.params.id];
    let query =
      'SELECT modified_time FROM instruments WHERE instrument_id = $1';
    try {
      const results = await pool.query(query, params);
      if (results.rowCount == 0) {
        res.status(204).json('No rows returned');
  ...
  static async EditTradeable(req, res, next) {
    try {
      let params = [req.params.id];
      let query =
        'UPDATE instruments SET isTradeable = FALSE modified_time = CURRENT_TIMESTAMP(0) WHERE instrument_id = $1'; // update a softdeleted column
      const results = await pool.query(query, params);
      if (results.rowCount == 0) {
  ...
  static async EditCountry(req, res, next) {
    ...
    try {
      ...
      let query =
        'UPDATE instruments SET country = $1 modified_time = CURRENT_TIMESTAMP(0) WHERE instrument_id = $2'; // update a softdeleted column
      const results = await pool.query(query, params);
      if (results.rowCount == 0) {
        res.status(500).json('No rows changed.');
      } else {
  ...
  static async EditSector(req, res, next) {
    ...
    let query =
      'UPDATE instruments SET sector = $1 modified_time = CURRENT_TIMESTAMP(0) WHERE instrument_id = $2'; // update a softdeleted column
    try {
      const results = await pool.query(query, params);
      if (results.rowCount == 0) {
        res.status(500).json('No rows changed.');
  ...
  static async EditNotes(req, res, next) {
    ...
    let query =
      'UPDATE instruments SET notes = $1 modified_time = CURRENT_TIMESTAMP(0) WHERE instrument_id = $2'; // update a softdeleted column
    try {
      const results = await pool.query(query, params);
      if (results.rowCount == 0) {
        res.status(500).json('No rows changed.');
